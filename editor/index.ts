...
function main() {
  ...
  changeHandleDiameter = initHandlePicker(
    hammer,
    elements.handleDiameterPicker,
    'handleDiameter',
    perspectiveDefault.handleDiameter,
  ).changeVal
  changeHandleLength = initHandlePicker(
    hammer,
    elements.handleLengthPicker,
    'handleLength',
    perspectiveDefault.handleLength,
  ).changeVal
...
function initHandlePicker(
  hammer: Hammer,
  handlePicker: Element,
  handleProp: 'handleDiameter' | 'handleLength',
  defaultValue?: number,
) {
...
function initDarkBackground(darkBackground: Element) {
  createCheckboxInput({
    elem: darkBackground,
...
function initDownloadHead(downloadHead: HTMLButtonElement, hammer: Hammer) {
  downloadHead.onclick = () => {
    ...
    setTimeout(() => {
      const content = generateSvgContent()
      downloadFile(content, 'image/svg+xml', 'vike-generated-head.svg')
